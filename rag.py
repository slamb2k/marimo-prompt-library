...
def create_rag():
    rag = Quart(__name__)
    rag.register_blueprint(bp)
    rag.config["TEMPLATES_AUTO_RELOAD"] = True
    return rag
...
# Method of Performing RAG
RAG_METHOD = os.environ.get("RAG_METHOD", 'aisearch')
...
AZURE_SEARCH_MAX_TOKENS =  os.environ.get("AZURE_SEARCH_MAX_TOKENS", 8000)
...
AZURE_OPENAI_EMBEDDING_NAME = os.environ.get("AZURE_OPENAI_EMBEDDING_NAME", "text-embedding-ada-002")
...
def retrieve_documentation(question):
    ...
    embedding = client.embeddings.create(input=question, model=AZURE_OPENAI_EMBEDDING_NAME).data[0].embedding 
    search_client = SearchClient(endpoint = f"https://{AZURE_SEARCH_SERVICE}.search.windows.net",  
                                  index_name = AZURE_SEARCH_INDEX,  
                                  credential=DefaultAzureCredential())   
    vector_query = VectorizedQuery(vector=embedding, k_nearest_neighbors=3, fields=AZURE_SEARCH_VECTOR_COLUMNS)   
    content =  search_client.search(  
        search_text=question,  
        top=5,  
        search_fields=[AZURE_SEARCH_CONTENT_COLUMNS],  
...
async def send_chat_request_aisearch(request):
    filtered_messages = []
    messages = request.get("messages", [])
    for message in messages:
        if message.get("role") != 'tool':
    ...
    try:
        azure_openai_client = init_openai_client()
        results = retrieve_documentation(request['messages'][0]['content'])
        docs = []  
        token_count = 0  
        max_tokens = AZURE_SEARCH_MAX_TOKENS
    
        for doc in results:  
            doc_tokens = len(doc[AZURE_SEARCH_TITLE_COLUMN]+doc[AZURE_SEARCH_URL_COLUMN]+doc[AZURE_SEARCH_CONTENT_COLUMNS])
            if token_count + doc_tokens <= max_tokens:  
                docs.append({"title": doc[AZURE_SEARCH_TITLE_COLUMN], "url": doc[AZURE_SEARCH_URL_COLUMN], "content": doc[AZURE_SEARCH_CONTENT_COLUMNS]})  
                token_count += doc_tokens  
            else:  
                break  
        context = {'citations': []}  
        for idx, each in enumerate(docs, start=1):  
                citation = {  
                    'doc_id':str(idx),
        ...
        prompt="""  
    You are an AI assistant that helps people find information. If the following documents are not related to this question, don't include the citation and simply write: "I'm sorry, but I cannot provide information without retrieved documents. Please provide relevant documents, and I will be happy to help you with your question." 
    If the following documents are related to the question. Please help to answer the question, include citation sources in the response and must use [docN] notation to indicate references in the answer, where N is doc_id of the document in the provided reference list.
    {source}
    """  
        prompt = prompt.format(source=context['citations'])  
        ...
        raise e
    return response, apim_request_id, context
...
async def complete_chat_request(request_body):
    if USE_PROMPTFLOW and PROMPTFLOW_ENDPOINT and PROMPTFLOW_API_KEY:
        ...
        return format_pf_non_streaming_response(
            ...
            try:  
                if RAG_METHOD == 'aisearch':  
                    global SHOULD_USE_DATA  
                    SHOULD_USE_DATA = False  
                    response, apim_request_id, source = await send_chat_request_aisearch(request_body)  
                    history_metadata = request_body.get("history_metadata", {})  
                    return format_non_streaming_response(response, history_metadata, apim_request_id, source)  
                elif RAG_METHOD == 'aoai':
                ...
                return {"error": "Invalid method. Please select a valid method between aoai and aisearch."}  

  
async def stream_chat_request(request_body):  
    if RAG_METHOD == 'aoai':  
        logging.debug('The RAG Method is AOAI')  
        response, apim_request_id = await send_chat_request(request_body)  
        history_metadata = request_body.get("history_metadata", {})  
    ...
    elif RAG_METHOD == 'aisearch':  
        logging.debug('The RAG Method is AI Search')  
        global SHOULD_USE_DATA  
        SHOULD_USE_DATA = False  
        response, apim_request_id, doc = await send_chat_request_aisearch(request_body)  
        history_metadata = request_body.get("history_metadata", {})  
        ...
        async def generate(): 
            ...
            async for completionChunk in response:  
                if completionChunk.id:  
                    if first_time and len(completionChunk.choices) > 0:  
                        yield format_stream_response(completionChunk, history_metadata, apim_request_id, doc)  
                        first_time = False  
                    else:  
                        yield format_stream_response(completionChunk, history_metadata, apim_request_id)  
